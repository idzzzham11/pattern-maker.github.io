...
class PatternInstructions {
    ...
    return [
      _buildStepBox(
        ...
        [
          ...
          '• Lebar (Width): 1/4 Bust = $bustQuarter cm',
          '• Panjang (Length): Back Length = ${m.backCenter} cm',
      ...
      _buildStepBox(
        ...
        [
          ...
          '• Bust Line: ${m.armhole} cm dari bahu',
      ...
      _buildStepBox(
        ...
        [
          ...
          '• Dart suppression diperlukan: $dartSuppression cm per side',
          '• Bahagikan dart: 40% di side seam, 60% di front/back dart',
          '• Front dart: Dari bust point, turun ke waist (panjang ${(double.parse(dartSuppression) * 0.6).toStringAsFixed(1)} cm)',
          '• Side dart: Dari side seam di bust, ke waist (panjang ${(double.parse(dartSuppression) * 0.4).toStringAsFixed(1)} cm)',
  ...
  static String _getSleeveInstructions(String sleeve, Measurements m) {
    final instructions = {
      'Sleeveless':
          'Tiada lengan. Finish armhole dengan facing atau bias binding. Armhole depth: ${m.armhole} cm.',
      'Short':
          'Panjang sleeve: 15-20 cm dari shoulder. Sleeve cap height: 1/3 armhole depth. Ease 2-3 cm di cap untuk smooth set-in.',
      'Long':
          'Panjang: ${m.sleeveLength} cm. Tambah 2 dart di elbow (depth 1.5 cm each). Wrist width: 18-20 cm dengan cuff.',
      'Puff':
  ...
  static String _getSkirtInstructions(
    ...
    final instructions = {
      'Straight':
          'Skirt lurus dari hip ke hem. Waist: $waistQ cm, Hip: $hipQ cm. Tambah dart (2-3 cm) di waist untuk fitting. Side seam straight.',
      'A-Line':
          'Flare dari waist. Hip width: $hipQ cm. Hem width: ${(double.parse(hipQ) + 10).toStringAsFixed(1)} cm. Flare beransur dari hip ke hem dengan smooth line.',
      'Flared':
